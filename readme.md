...
~~~
docker run --name mypostgres -e POSTGRES_PASSWORD=test -p 5432:5432 -d lordpretzel/cs480
~~~
...
~~~
docker run --name mypostgres -e POSTGRES_PASSWORD=test -v /mypost_data:/var/lib/postgresql/data -p 5432:5432 -d lordpretzel/cs480
~~~

The `-v` parameter mounts local directory `/mypost_data` as directory `/var/lib/postgresql/data` in the container. Directory `/var/lib/postgresql/data` is the location which the `lordpretzel/cs480` image uses to store the postgres cluster directory.
...
~~~
docker run -it --rm --link mypostgres:postgres lordpretzel/cs480 psql -h postgres -U postgres -d cs480
~~~
